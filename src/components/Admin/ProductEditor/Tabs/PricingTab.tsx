      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            />
            <div className="text-sm text-gray-500 mt-1 space-y-1">
              <p>Porcentagem máxima de desconto que o cliente pode aplicar usando UTI Coins</p>
              {formData.price && formData.uti_coins_discount_percentage && (
                <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 p-2 rounded border border-blue-200 dark:border-blue-800">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 bg-gradient-to-br from-blue-400 to-cyan-500 rounded-full flex items-center justify-center">
                      <span className="text-white font-bold text-xs">D</span>
                    </div>
                    <span className="text-blue-800 dark:text-blue-200 font-medium">
                      Desconto máximo: {formatCurrency((formData.price * formData.uti_coins_discount_percentage) / 100)} 
                      ({Math.round((formData.price * formData.uti_coins_discount_percentage) * 100).toLocaleString()} UTI Coins)
                    </span>
                  </div>
                </div>
              )}
