import React, { useState, TouchEvent } from 'react'; // Adicionado TouchEvent
...
const MainHeader = ({
  ...
  // Handler para o botão de menu mobile, usando onTouchEnd
  const handleMenuToggleTouch = (e: TouchEvent<HTMLButtonElement>) => {
    e.preventDefault(); // Previne clique simulado
    onMobileMenuToggle(); // Chama a função original
  };

  // Handler para o botão de busca mobile, usando onTouchEnd
  const handleSearchToggleTouch = (e: TouchEvent<HTMLButtonElement>) => {
    e.preventDefault(); // Previne clique simulado
    toggleMobileSearch(); // Chama a função original
  };
      ...
      <div className={cn("bg-background border-b sticky top-0 z-40", className)}> 
        <div className="container flex h-[72px] items-center justify-between px-4 gap-2">
          <div className="flex items-center flex-shrink min-w-0"> 
            {/* Mobile Menu Toggle Button com onTouchEnd */}
            <Button
              variant="ghost"
              size="icon"
              className="mr-2 md:hidden p-2 h-10 w-10 flex-shrink-0"
              onClick={onMobileMenuToggle} // Mantém para desktop/acessibilidade
              onTouchEnd={handleMenuToggleTouch} // Adiciona para mobile
          ...
          <div className="flex items-center justify-end flex-shrink-0 gap-1"> 
            {/* Mobile Search Toggle Button com onTouchEnd */}
            <Button
              variant="ghost"
              size="icon"
              className="md:hidden p-2 h-10 w-10"
              onClick={toggleMobileSearch} // Mantém para desktop/acessibilidade
              onTouchEnd={handleSearchToggleTouch} // Adiciona para mobile
