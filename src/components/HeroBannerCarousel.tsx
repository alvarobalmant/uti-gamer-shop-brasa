...
const HeroBannerCarousel = () => {
  ...
  return (
    <section className="relative bg-gray-900 overflow-hidden">
      <div 
        ref={carouselRef} 
        className={`relative text-white transition-all duration-500 ease-in-out transform ${
          backgroundType === 'image-only' ? 'bg-gray-800' : `bg-gradient-to-br ${banner.gradient}`
        }`}
        style={{
            ...
            {banner.button_text && banner.button_link && (
              ...
              >
                {banner.button_image_url && (
                  <img src={banner.button_image_url} alt="" className="w-4 h-4 mr-2" />
                )}
                {banner.button_text}
              </Button>
            )}
