...
const PromotionalBanner: React.FC<PromotionalBannerProps> = ({
  ...
  return <div className={cn("rounded-lg overflow-hidden shadow-lg my-6 md:my-8",
  // Margin top/bottom
  backgroundColor)}>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-0 items-center">
        ...
        <div className="md:col-span-3 p-4 md:p-8 lg:p-10 flex flex-col justify-center order-2 md:order-1">
          <h2 className={cn("text-xl md:text-3xl lg:text-4xl font-bold mb-1 md:mb-3",
        // Reduced font size and margin bottom on mobile
        textColor)}>
            {title}
          </h2>
          <p className={cn("text-sm md:text-lg mb-3 md:mb-6 opacity-90",
        // Reduced font size and margin bottom on mobile
        textColor)}>
            {description}
          </p>
          <a href={buttonLink} target="_blank" rel="noopener noreferrer" className="self-start">
            <Button variant="outline" // Outline style often works well on dark backgrounds
          size="sm" // Reduced button size on mobile
          className={cn("bg-transparent border-white/80 hover:bg-white/10 active:bg-white/20 text-xs md:text-sm",
        ...
        <div className="md:col-span-2 h-16 sm:h-20 md:h-full order-1 md:order-2"> {/* Reduced height for mobile to maintain rectangular shape */}
          <img src={imageUrl || "/placeholder-banner.webp"} alt={title} onError={e => {
          e.currentTarget.onerror = null;
          e.currentTarget.src = "/placeholder-banner-error.webp";
        }} loading="lazy" className="w-full h-full object-cover md:object-scale-down" />
        </div>
      </div>
    </div>;
};
