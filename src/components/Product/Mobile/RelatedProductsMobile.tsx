...
const RelatedProductsMobile: React.FC<RelatedProductsMobileProps> = ({ product }) => {
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      try {
        ...
        if (!error && data) {
          setRelatedProducts(data.map(mapIntegraRowToProduct));
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="bg-white p-4">
        <SectionTitle title="Produtos relacionados" showViewAllButton={false} className="mb-4" />
  ...
  return (
    <div className="bg-white p-4">
      <SectionTitle title="Produtos relacionados" onViewAllClick={() => {}} className="mb-4" />
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4">
        {relatedProducts.map((relatedProduct) => (
          <Card
            key={relatedProduct.id}
            className="flex-shrink-0 w-40 h-64 relative flex flex-col bg-white overflow-hidden border border-gray-200 rounded-lg cursor-pointer p-0"
            onClick={() => navigate(`/produto/${relatedProduct.id}`)}
          >
            <ProductCardBadge text={relatedProduct.badge_text || ''} color={relatedProduct.badge_color || '#22c55e'} isVisible={relatedProduct.badge_visible || false} />
