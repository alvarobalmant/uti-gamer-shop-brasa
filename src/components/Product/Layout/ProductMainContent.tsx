    ...
    return (
      <div className="space-y-6 max-w-md">
        ...
        <div>
          ...
          {/* UTI Coins Section - DEBUG VERSION */}
          <div className="mt-4 space-y-3">
            {/* Debug info */}
            <div className="bg-gray-100 p-2 text-xs rounded">
              Debug: cashback={product.uti_coins_cashback_percentage || 'undefined'}, discount={product.uti_coins_discount_percentage || 'undefined'}
            </div>
            
            {/* Cashback UTI Coins - sempre mostrar para teste */}
            <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200 rounded-lg p-3">
              <div className="flex items-center gap-2">
                <span className="text-lg">💰</span>
                <div>
                  <p className="text-sm font-medium text-yellow-800">
                    Ganhe {Math.round((product.price * (product.uti_coins_cashback_percentage || 5) / 100) * 100)} UTI Coins nesta compra
                  </p>
                  <p className="text-xs text-yellow-600">
                    Cashback de {product.uti_coins_cashback_percentage || 5}% em UTI Coins
                  </p>
                </div>
              </div>
            </div>

            {/* Desconto UTI Coins - sempre mostrar para teste */}
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-3">
              <div className="flex items-center gap-2">
                <span className="text-lg">🎯</span>
                <div>
                  <p className="text-sm font-medium text-purple-800">
                    Até {product.uti_coins_discount_percentage || 15}% OFF pagando com UTI Coins
                  </p>
                  <p className="text-xs text-purple-600">
                    Economize até R$ {(product.price * (product.uti_coins_discount_percentage || 15) / 100).toFixed(2).replace(".", ",")}
                  </p>
                </div>
              </div>
            </div>
