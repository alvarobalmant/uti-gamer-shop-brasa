...
-- 4. TRIGGERS para auto-atualização
...
-- 7. TRIGGER para criar conta automaticamente
...
-- 9. INSERIR ADMIN padrão (se não existir)
DO $$
BEGIN
  -- Verificar se já existe admin
  IF NOT EXISTS (SELECT 1 FROM profiles WHERE role = 'admin') THEN
    -- Inserir admin padrão na tabela profiles apenas
    INSERT INTO profiles (id, name, email, role) 
    VALUES (
      'admin-default-id'::UUID, 
      'Administrador', 
      'admin@utidosgames.com', 
      'admin'
    );
    
    RAISE NOTICE 'Admin padrão criado: admin@utidosgames.com';
  END IF;
END $$;