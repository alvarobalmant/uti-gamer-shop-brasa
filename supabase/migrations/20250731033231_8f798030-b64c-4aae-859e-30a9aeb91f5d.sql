-- Fixed Plan Implementation: Fix products.idasproduct_id error
...
-- Step 2: Force PostgREST schema reload by sending notification
SELECT pg_notify('pgrst', 'reload schema');

-- Step 3: Create simple diagnostic function with correct types
...
BEGIN
  ...
  SELECT 
    ...
    (schemaname || '.' || viewname)::text as source_name,
    (definition LIKE '%idasproduct_id%' OR 
     definition LIKE '%idas%product_id%' OR
     definition LIKE '%product_id%id%')::boolean as has_problematic_ref
  FROM pg_views 
  WHERE schemaname = 'public';
  
  -- Check functions with text casting
  RETURN QUERY
  SELECT 
    'function'::text as source_type,
    p.proname::text as source_name,
    (pg_get_functiondef(p.oid)::text LIKE '%idasproduct_id%' OR 
     pg_get_functiondef(p.oid)::text LIKE '%idas%product_id%' OR
     pg_get_functiondef(p.oid)::text LIKE '%product_id%id%')::boolean as has_problematic_ref
  FROM pg_proc p
...
SELECT * FROM debug_column_references() WHERE has_problematic_ref = true;