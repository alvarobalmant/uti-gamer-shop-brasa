-- Atualizar a função complete_order_verification para processar descontos UTI Coins
...
DECLARE
  ...
  -- NOVO: Variáveis para desconto UTI Coins
...
BEGIN
  ...
  -- NOVO: Buscar saldo atual de UTI Coins do usuário
  ...
  -- CALCULAR DESCONTOS UTI COINS - PRIORIDADE: UTI PRO primeiro, depois UTI Coins
  ...
  -- CALCULAR CASHBACK EM UTI COINS (FUNCIONAMENTO NORMAL)
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_order_record.items)
  LOOP
    -- Extrair informações do item
    v_item_total := (v_item->>'total')::NUMERIC;
