-- 🚀 Backend Performance Optimizations for UTI dos Games
-- This migration implements critical performance improvements identified in the analysis
...
-- Index for special_sections table
CREATE INDEX IF NOT EXISTS idx_special_sections_title 
ON special_sections (title);

-- GIN index for JSONB fields in special_section_elements (if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'special_section_elements') THEN
    CREATE INDEX IF NOT EXISTS idx_special_section_elements_content_gin 
    ON special_section_elements USING GIN (content);
  END IF;
END $$;
...
-- First, add critical indices for the view optimization
...
-- Now recreate the view with better performance
...
-- Drop existing policies that might be creating performance issues
...
COMMENT ON VIEW view_product_with_tags IS 
'Optimized product view with proper indexing. Cache for 2-5 minutes depending on update frequency.';
...
BEGIN
    ...
    RAISE NOTICE '✅ view_product_with_tags optimized with better query plan';
